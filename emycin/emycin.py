...
class Shell(object):
    ...
    def __init__(self, read=raw_input, write=write, ui=None):
        """
        ...
        # ---- my stuff -----------------------------------
        self.ui = ui
        self.write = self.ui.insert_text
...
<val1> <cf1> [, <val2> <cf2>, ...]
    ...
    @staticmethod
    def get_question(param, inst):
        if param == 'district':
            return 'In which district the land is in?'
        elif param == 'ground-slop':
            return 'How much is the ground slope (degrees)?'
        elif param == 'has-contour':
            return 'Does the land contain contours?'
        elif param == 'has-building':
            return 'Does the land contain a building?'
        elif param == 'has-retaining-wall':
            return 'Does the vertical cut has a retaining wall?'
        elif param == 'has-vertical-cut':
            return 'Does a vertical cut exist in the land?'
        elif param == 'vertical-cut-height':
            return "What's the vertical cut height (m)?"
        elif param == 'distance-from-house-to-vertical-cut':
            return "How long is it from the vertical cut to the house (m)?"
        elif param == 'dimension-perpendicular-to-contour':
            return 'What is the height (m) in the perpendicular direction to contour?'
        elif param == 'soil-type':
            return 'What is the soil type?'
        elif param == 'suggestion':
            return 'Any known suggestions?'
        elif param == 'NBRO-approval':
            return 'Is the NBRO approval status known?'
        else:
            return 'Undefined question? - {0}'.format(param)
    ...
    def ask_values(self, param, inst):
        ...
        while True:
            ...
            # resp = self.ui.get_input('What is the %s of %s-%d? ' % (param, inst[0], inst[1]))
            resp = self.ui.get_input(self.get_question(param, inst[0]))
    ...
    def execute(self, context_names):
        ...
        """
        self.ui.disable_reset_button()
        logging.info('Beginning data-gathering for %s' % ', '.join(context_names))

        self.write('For help answering questions, press "help". Also press "Unknown" if not applicable')
        self.clear()
        results = {}
